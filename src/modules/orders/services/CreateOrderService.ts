...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistentProducts.length !== 0) {
      throw new AppError(
        `Could not find product with ids ${checkInexistentProducts.join(', ')}`,
      );
    }
    ...
    if (findProductsWithNoQuantityAvailable.length !== 0) {
      const message = findProductsWithNoQuantityAvailable.map(
        product =>
          `The quantity ${product.quantity} is not avaliable for ${product.id}`,
      );
